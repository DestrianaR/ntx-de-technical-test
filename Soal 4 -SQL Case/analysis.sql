-- Note: From https://support.google.com/analytics/answer/3437719?hl=en Please note that the `transactionRevenue` column is no longer used, `totalTransactionRevenue` is used instead and the `totalTransactionRevenue` column will be divided by 1000000 to show the actual figure.
...
-- There are 562 users who spend above-average time on the site but view fewer pages than the average user. However, only two users have non-null values in the avgSessionQualityDim column, indicating that only these two users had high session quality (i.e., were very close to making a transaction). Therefore, the longer avgTimeOnSite does not have a significant impact on the likelihood of a user completing a transaction.
